...
//dd($request->all());
    ...
    public function request_money(Request $request)
    {
        ...
        if($existsRequest==0){
            ...
            $paymentRequest->requested_amt = $request->input('requested_amt');
            $paymentRequest->save();
            
            return back()->with('flash_success','Request Submitted. Please wait for our staffs to review.');
        }
        else{
            return back()->with('flash_error','Request Already Submitted.');
        }
    }
 ...
 public function getBill(){
    $bill = billUpdate::where('provider_id',Auth::user()->id)->orderby('created_at','asce')->get();
