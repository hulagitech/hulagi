...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        try{
            ...
            $inKtm=UserRequests::where('zone1',$zoneKtm)
                            ->whereColumn('zone1','zone2')
                            ->where('created_at','LIKE',date("Y-m-d").'%')
                            ->count();
            $outKtm=UserRequests::where('zone1','!=',$zoneKtm)
                            ->whereColumn('zone1','zone2')
                            ->where('created_at','LIKE',date("Y-m-d").'%')
                            ->count();
            $interCity=UserRequests::whereColumn('zone1','!=','zone2')
                            ->where('created_at','LIKE',date("Y-m-d").'%')
                            ->count();
            
            $total_inKtm=UserRequests::where('zone1',$zoneKtm)
                            ->whereColumn('zone1','zone2')
                            ->count();
            $total_outKtm=UserRequests::where('zone1','!=',$zoneKtm)
                            ->whereColumn('zone1','zone2')
                            ->count();
            $total_interCity=UserRequests::whereColumn('zone1','!=','zone2')
                            ->count();
            // $stats = UserRequests::where('created_at','LIKE',date("Y-m-d").'%')
            ...
            // ])
            // ->get();
            $stats=DB::table('user_requests')
            ->join('users', function ($join) {
                $join->on('user_id', '=', 'users.id');
            })->where('user_requests.created_at','LIKE',date("Y-m-d").'%')
            ->groupBy('user_id')
                ->select([
                    DB::raw('users.first_name as user'),
            ...
            $total_stats = UserRequests::where('created_at', '>=', $range)
            ->groupBy('date')
            ->orderBy('date', 'DESC')
            ->select([
            ...
            ->get();
            return view('admin.dashboard',compact('scheduled_rides','rides','cancel_rides','stats','pending_rides','completed_rides', 'rejected_rides','picked_rides','accepted_rides', 'sortcenter', 'delivering', 'assigned', 'total_rides', 'total_cancel_rides', 'total_pending_rides', 'total_completed_rides', 'total_rejected_rides', 'total_scheduled_rides', 'total_picked_rides', 'total_accepted_rides', 'total_sortcenter', 'total_delivering', 'total_assigned','inKtm','outKtm','interCity','total_inKtm','total_outKtm','total_interCity','total_stats'));
        }
        catch(Exception $e){
            dd($e->getMessage());
...
private static function last7DaysTripRe(){
  ...
  if (!$last7days_rides->isEmpty()) {
    ...
    {
        try{
            $rides = UserRequests::has('user')->where('status','PENDING')->orderBy('id','desc')->get();
    ...
    public function settings_store(Request $request)
    {
        ...
        $this->validate($request,[
                ...
                'white_site_logo' => 'mimes:jpeg,jpg,bmp,png|max:5242880',
            ]);
        ...
        if($request->hasFile('white_site_logo')) {
            $site_logo = Helper::upload_picture($request->file('white_site_logo'));
            Setting::set('white_site_logo', $site_logo);
        }
        ...
        Setting::set('social_login_driver', $request->social_login_driver);
        ...
        /*Setting::set('referral_point', $request->referral_point);*/
    ...
    public function settings_payment_store(Request $request)
    {
        ...
        Setting::set('cargo_amount', $request->cargo_amount);
        Setting::set('hold_amount', $request->hold_amount);
        Setting::save();
    ...
    public function statement_provider(){
        ...
        try{
            ...
            /*$Providers = UserRequests::where('paid', 1)
                    ->has('user')
                    ->has('provider')
                    ->has('payment')
                    ->orderBy('user_requests.created_at','desc')
    ...
    public function changeAccountPassword(Request $request)
    {
        ...
        try {
            ...
            $provider = Account::findOrFail($request->id);
			if( !$provider  ) {
    ...
    public function changeReturnPassword(Request $request)
    {
        if(Setting::get('demo_mode', 0) == 1) {
            return back()->with('flash_error','Disabled for demo purposes! Please contact us at info@appoets.com');
        }
        ...
        try {
            ...
            $provider = ReturnUser::findOrFail($request->id);
			if( !$provider  ) {
				throw new Exception('User Not Found');
			}
    ...
    public function changeSupportPassword(Request $request)
    {
        if(Setting::get('demo_mode', 0) == 1) {
            return back()->with('flash_error','Disabled for demo purposes! Please contact us at info@appoets.com');
        }
        ...
        try {
            ...
            $provider = SupportUser::findOrFail($request->id);
			if( !$provider  ) {
				throw new Exception('User Not Found');
			}
    ...
    public function changeSortcenterPassword(Request $request)
    {
        if(Setting::get('demo_mode', 0) == 1) {
            return back()->with('flash_error','Disabled for demo purposes! Please contact us at info@appoets.com');
        }
        ...
        try {
            ...
            $provider = SortCenterUser::findOrFail($request->id);
			if( !$provider  ) {
				throw new Exception('User Not Found');
			}
    ...
    public function changePickupPassword(Request $request)
    {
        if(Setting::get('demo_mode', 0) == 1) {
            return back()->with('flash_error','Disabled for demo purposes! Please contact us at info@appoets.com');
        }
        ...
        try {
            ...
            $provider = PickupUser::findOrFail($request->id);
			if( !$provider  ) {
				throw new Exception('User Not Found');
			}
    ...
    public function is_read_post(){
       try {

            $data =  User::where('is_read',0)->orderBy('id', 'desc')->get();
            foreach ($data as $key => $value) {
                $user = User::where('id',$value->id)->first();
                $user->is_read = 1;
                $user->save();
            }
            $data1 =  Provider::where('is_read',0)->orderBy('id', 'desc')->get();
            foreach ($data1 as $key => $value1) {
                $user1 = Provider::where('id',$value1->id)->first();
                $user1->is_read = 1;
                $user1->save();
            }
            return back();
    ...
    public function track(){
        return view('admin.request.track');
    ...
    public function trackPost(Request $request){
        try {
            ...
            $user_request= UserRequests::where('booking_id',$request->searchField)->first();
            
            $order_id=$user_request->id;
                $rider_history=AssignedRidersHistory::where('request_id',$order_id)->get();
            // return $order="hi";
            return view('admin.request.track',compact('rider_history'));
        }else{
            return view('admin.request.track');
        }
    ...
    public function ReceivedRemainingDispatch()
    {
        try {
            ...
            return view('admin.dispatcher.page', compact(['requests']));
        }   catch (Exception $e) {
            return back()->with('flash_error','Something Went Wrong!');
        }
    ...
    public function logs($id){
        try{
            ...
            return view('admin.request.logs', compact('logs'));
        }
        catch(Exception $e){
            return back()->with('flash_error', 'Something went wrong!!!');
        }
    }
    public function KtmDeliveryRemaining(){

        try {
             $zoneKtm=Zones::where('zone_name','Kathmandu')->select('id')->first()->id;

             $remaining_data=UserRequests::whereNotIn('status',['COMPLETED','REJECTED','CANCELLED','TOBERETURNED','RETURNED'])->whereDate('created_at', '<=', Carbon::today()->setTime(11, 00, 00)->toDateTimeString())->where('zone2',$zoneKtm)->get();
            //  dd($remaining_data);
            return view('admin.request.ktm_delivery_remaining',compact('remaining_data'));
        } 
        catch (Exception $e) {
            return back()->with('flash_error', 'Order Not Found');
        }}
    public function insidevalley(Request $request)
    {
        $zoneKtm=Zones::where('zone_name','Kathmandu')->select('id')->first()->id;
        ...
        try {
            if(isset($request->search)){
                ...
                return view('admin.request.insidevalley', compact(['requests', 'dates']));

            }
            else{
            //$requests = UserRequests::whereIn('status',['COMPLETED'])->RequestHistory()->get();
            $requests = UserRequests::where('zone2',$zoneKtm)
                                        ->whereRaw('DATE(created_at) = CURRENT_DATE')
                                        ->RequestHistory()->paginate(100);
    ...
    public function outsidevalley(Request $request)
    {
        $zoneKtm=Zones::where('zone_name','Kathmandu')->select('id')->first()->id;
        ...
        try {
            if(isset($request->search)){
                ...
                $current['date']=$request->date;
                return view('admin.request.outsidevalley', compact(['requests', 'dates']));
    ...
    public function wallet(){
        try{
            ...
            return view('admin.users.negative',compact('users'));
        }
        catch (Exception $e) {
            return back()->with('flash_error','Something Went Wrong!');
        }
    ...
    {
        try{
            $requests=UserRequests::whereIn('status',['SORTCENTER','SCHEDULED','DISPATCHED','DELIVERING'])
                                ->whereRaw('TIMESTAMPDIFF(day, updated_at, CURRENT_TIMESTAMP) > 1')
                                ->whereRaw('TIMESTAMPDIFF(day, updated_at, CURRENT_TIMESTAMP) > 1')
                                ->paginate(300);

            return view('admin.request.inactive',compact(['requests']));
        }
        catch(Exception $e){
            return $e->getMessage();
            return back()->with('flash_error','Something Went Wrong!');
    ...
    public function send_mail()
    {
        try{
            ...
            // dd($users);
            foreach($users as $user){
                $token=Helper::generate_esewa_id();
                $esewa=new EsewaPaymentTroughMail;
                // dd($user->email);
                $esewa->email=$user->email;
                $esewa->user_id=$user->id;
                $esewa->Name=$user->first_name;
                $esewa->Amount=abs($user->newPayment);
                $esewa->EsewaToken=$token;
                $esewa->save();
                Notification::send($esewa, new PaymentNotification($esewa));
            }
            return redirect()->route('admin.user.index')->with('flash_success','Mail has been sent successfully');
                    // return view('admin.user.mail')->with('flash_success','Mail Has been Sent');
        }
        catch(Exception $e){
            return redirect()->route('admin.user.index')->with('flash_error','Something Went Wrong with Dashboard!');
        }
    ...
    public function getOrderDetails($id)
    {
        try {
            ...
            // }
            return view('admin.layout.order', compact('request','promocode', 'comments','logs'));
        }   catch (Exception $e) {
