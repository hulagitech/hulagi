...
class SendPushNotification extends Controller
{
    ...
    //dd($img);
        ...
        shell_exec('curl -X POST --header "Authorization: key=AAAARiuQ8YA:APA91bFYIczsr0SoHqgaqaimTYttcFlmx0qGGAYEgQUFYVjfkAtBUODsZX1TGpMR00i9946EudB0vS0nE0HECN54gguMhL_DxEKLHi1gk0YkpwdMRcsVrTtkFeZxXWdt-_R1oQecX7IB" --header "Content-Type: application/json" https://fcm.googleapis.com/fcm/send -d "{\"to\":\"'.$user->device_token.'\",\"priority\":\"high\",\"data\":{\"msg_type\":\"'.$msg_type.'\",\"request_id\":\"'.$request_id.'\",\"image_url\":\"'.$img.'\",\"user_name\":\"'.$username.'\",\"msg\":\"'.$push_message.'\"},\"notification\":{\"body\": \"'.stripslashes($push_message).'\",\"title\":\"'.$msg_type.'\",\"image\":\"'.$img.'\"}}"');
            ...
            if($provider->token != ""){
            	if($provider->type == 'ios'){
            		/*return \PushNotification::app('IOSProvider')
        	            ->to($provider->token)
        	            ->send($push_message);*/
        shell_exec('curl -X POST --header "Authorization: key=AAAARiuQ8YA:APA91bFYIczsr0SoHqgaqaimTYttcFlmx0qGGAYEgQUFYVjfkAtBUODsZX1TGpMR00i9946EudB0vS0nE0HECN54gguMhL_DxEKLHi1gk0YkpwdMRcsVrTtkFeZxXWdt-_R1oQecX7IB" --header "Content-Type: application/json" https://fcm.googleapis.com/fcm/send -d "{\"to\":\"'.$provider->token.'\",\"priority\":\"high\",\"data\":{\"msg_type\":\"'.$msg_type.'\",\"request_id\":\"'.$request_id.'\",\"image_url\":\"'.$img.'\",\"user_name\":\"'.$username.'\",\"msg\":\"'.$push_message.'\"},\"message_type\":\"chat\",\"notification\":{\"body\": \"'.stripslashes($push_message).'\",\"title\":\"'.$msg_type.'\",\"image\":\"'.$img.'\"}}"');	            
