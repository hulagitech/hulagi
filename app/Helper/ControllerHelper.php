...
class Helper
{
    ...
    public static function generate_booking_id() {
        while(true){
            $id=Setting::get('booking_prefix').mt_rand(1000000, 9999999);
            $exists=UserRequests::where('booking_id',$id)->count();
            if($exists==0){
                return $id;
            }
            continue;
    ...
    public static function generate_esewa_id() {
        while(true){
            $id="HULAGISEWA".mt_rand(100000000, 999999999);
            $exists1=Esewa::where('Payment_ID',$id)->count();
            $exists2=EsewaPaymentTroughMail::where('EsewaToken',$id)->count();
