    ...
    <div class="p-invoice">
        ...
        <div class="col-lg-12">
            <table width="100%" class="vr_details" >
                <tr>
                    <td width="60%"> <img class="puryau-logo" style="position:absolute;top:-10px;left:1cm;width:135px" src="{{ url(Setting::get('site_logo', asset('logo-black.png'))) }}"> </> </td>
                    <td width="20%"> </td>
                    <td width="22%"> 
                        <div style="font-size:18px;"> <b>  {{ $order->created_at->format('Y-m-d') }}</b>  </div>
                    </td>
                </tr>
            </table>
        </div>
            ...
            <div  style="width: 100%; margin-top:10px; display:flex;">
                <table style="width: 100%;" class="puryau_details">
                    <tr>
                        <td width="60%">
                            @if(@$order->user->user_type!='puryau')
                            <div class> <i class="fa fa-globe"></i> &nbsp; <b>{{ $order->user->domain->websitelink ?? 'www.hulagi.com'}}</b> </div>
                            <div> <i class="fa fa-map-marker"></i> &nbsp;&nbsp; {{ $order->user->domain->location ?? 'Dillibazar, Kathmandu'}} </div>
                            <div> <i class="fa fa-envelope-o"></i> &nbsp; {{ $order->user->domain->Email ?? 'hulagilogistics@gmail.com' }} </div>
                            <div> <i class="fa fa-phone"></i> &nbsp; {{ $order->user->domain->phone ?? '01-5912256/57'}} @if($order->cargo)
                                        &nbsp;<span style="border:solid black 1px;padding:1px 3px ;">**Cargo**</span>  
                                    @endif
                                    {{-- {{ $order->cargo }}  --}}</div>
                            @else
                            <div class> <i class="fa fa-globe"></i> &nbsp; <b>www.puryau.com</b> </div>
                            <div> <i class="fa fa-map-marker"></i> &nbsp;&nbsp; Shankhamul, Ktm </div>
                            <div> <i class="fa fa-envelope-o"></i> &nbsp; hello@puryau.com </div>
                ...
                <tr>
                    ...
                    <td width="60%" style="padding-left: 10px;">
                        ...
                        <div style="widt:50px;"> <i class="fa fa-map-marker"></i> &nbsp; : 
                            {{ $order->d_address }} </div>
                        {{-- @if(strlen($order->d_address)>20)
                            {{ substr($order->d_address) }} 
                        @else
                        {{ $order->d_address }}
                        @endif --}}
                    </div>
