    ...
    <div class="row">
        <div class="col-12">
            ...
            <tbody>
                ...
                <tr id="dataRow{{$index}}"  class="{{ @$inbound_order->user->wallet_balance< 0 ? 'text-danger' : '' }}">
                    <td >
                    ...
                    <td>
                        ...
                        <a href="{{ route('sortcenter.vendorInbound',$inbound_order->user->id) }}">
                            {{ @$inbound_order->user->first_name }} <b>({{@$inbound_order->user->user_type}})</b></a>
                        @else
                        N/A
                        @endif
                    </td>
                    ...
                    <td style="width:4%;">
                        @if($inbound_order->cargo)
                            <label class="switch">
                                <input type="checkbox" class="cargo" value="{{$inbound_order->cargo}}" @if(isset($inbound_order->cargo)) @if(@$inbound_order->cargo=='1') checked @endif @endif id='cargo-{{$inbound_order->id}}'>
                                <span class="slider round"></span>
                            </label>
                        @else
                            <label class="switch">
                                <input type="checkbox" value="{{$inbound_order->cargo}}" class="cargo" id='cargo-{{$inbound_order->id}}'>
                                <span class="slider round"></span>
                            </label>
                        @endif
                    </td>
                        <td>{{$inbound_order->status}}</td>
                    <td>
                        <div style="display:flex;">
                            <!-- <a style="height: fit-content;" href="#" class="allinbound btn btn-warning"
                                name="status-{{$inbound_order->id}}">Inbound</a>
                            &nbsp;
                            <span style="color: orange; display:none;" class="checkinbound"><i
                                    class="fa fa-check"></i></span> -->
                            @if($inbound_order->status=='NOTPICKEDUP')
                            Plese check this order status
                            @else
                            <form class="inboundAndPrint form-inline" target="_blank" method="POST"
                                action="{{url('sortcenter/printInvoice/'.$inbound_order->id)}}">
                                
                                <button type="submit" class="btn btn-primary">Print</button>
                                {{-- <a href="{{url('admin/printInvoice/'.$inbound_order->id)}}" target="_blank"
                            ...
                            <span style="color: orange; display:none;" class="checkinbound"><i
                                    class="fa fa-check"></i></span>
                            &nbsp;
                            <a href="{{ route('sortcenter.order_details', $inbound_order->id) }}"
...
<script>
    $(document).ready(function(){
        ...
        $(".txtedit").focusout(function(){
            ...
            $.ajax({
                url: "{{url('sortcenter/requests')}}/"+edit_id,
                type: 'post',
...
<script>
    $(document).ready(function(){
        ...
        $(".cargo").click(function(){
            ...
            $.ajax({
                url: "{{url('sortcenter/changeCargo')}}/"+edit_id,
                type: 'post',
                data: field_name+"="+value,
                success:function(response){
                    console.log(response); 
