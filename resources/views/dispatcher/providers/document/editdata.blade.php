...
<div class="content-area py-1">
    <div class="container-fluid">
    	<div class="box box-block bg-white">
            <a href="{{ url('dispatcher/provider/'.$Document['provider_id'].'/document') }}" class="btn btn-default pull-right"><i class="fa fa-angle-left"></i> Back</a>
			
			<h5 style="margin-bottom: 2em;">Add Driver Document</h5>

		
            <form class="form-horizontal" action="{{ url('dispatcher/provider/'.$Document['provider_id'].'/document/'.$Document['document_id'].'/update') }}" method="POST" enctype="multipart/form-data" role="form">
            	{{csrf_field()}}
            ...
            <h5 class="mb-1">Document: {{ $Document['document']['name'] }}</h5>
				<embed src="{{ asset('storage/'.$Document['url']) }}" width="300px" height="300px" />
				...
				<div class="form-group row">
					...
					<div class="col-xs-10">
						...
						<a href="{{ url('dispatcher/provider/'.$Document['provider_id'].'/document') }}" class="btn btn-default">Cancel</a>
					</div>
				</div>
			</form>
