		...
		<div class="page-title-box">
			<div class="row align-items-center">
				...
				<div class="col-md-8 d-flex justify-content-end">
					<form class="form-inline pull-right" method="GET" action={{url("dispatcher/dispatchList")}}>
						<label for="zone">Select Dispatcher to Dispatch:</label>
						<select class="form-control" name="dispatcher">
							@foreach ($dispatchers as $d)
								<option value="{{$d->id}}" {{(isset($searched_dispatcher) && $searched_dispatcher->id==$d->id)?"selected":null}}>{{$d->name}}</option>
							@endforeach
						</select>
 ...
 <div class="row">
        <div class="col-12">
            <div class="page-title-box">
                <div class="page-title-box bg-white">
						@if(!isset($searched_dispatcher))
						Please Search above for zone to dispatch
					@elseif($requests)
                    <div class="row align-items-center">
                        <div class="col-md-1"></div>
                        <h2 class="col-md-4">{{ date('d M Y') }}</h2>
                        <h2 class="col-md-4">{{$searched_dispatcher->name}}</h1>
                            <h3 class="col-md-2"><span id="dispatchCount">0</span>/{{$requests->count()}}</h3>
                            <div class="col-md-1"></div>
                    </div>
                    <form id="dispatchForm" method="POST" action="{{ url('dispatcher/dispatchList') }}" method="POST"
                        class="px-2">
                        {{ csrf_field() }}
						<input type="hidden" name="dispatcher" value="{{$searched_dispatcher->id}}"/>
                        <label>Orders</label>
					...
					@else
						No Orders for the dispatcher "{{$searched_dispatcher->name}}"
					@endif
            </div>
		...
		<script>
			var zones = new Bloodhound({
				...
				prefetch: {
					url: "{{isset($searched_dispatcher)?url('/dispatcher/dispatchOrders/'.$searched_dispatcher->id):null}}",
					cache: false
				},
			});
			zones.initialize();
