    ...
    <div class="container-fluid">
        <div class="box box-block bg-white">
            ...
            <div class="row">
                    ...
                    <table class="table table-striped table-bordered dataTable">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <form action="{{ route('bm.provider.document.service', [$Provider->id, $service->id]) }}" method="POST">
                                        {{ csrf_field() }}
                                        {{ method_field('DELETE') }}
                ...
                <form action="{{ route('bm.provider.document.store', $Provider->id) }}" method="POST">
                    {{ csrf_field() }}
                    <div class="col-xs-3">
					...
					<tbody>
						...
						<tr>
							...
							<td>
									...
									<a href="{{ url('bm/provider/'.$document['provider_id'].'/document/'.$document['id'].'/upload') }}" style="margin-left: 1em;" class="btn btn-primary pull-right"><i class="fa fa-plus"></i> Add Doccument</a>
									
									@else
										<a href="{{ route('bm.provider.document.edit', [$document['provider_id'], $document['id']]) }}"><span class="btn btn-success btn-large">View</span></a>
										<a href="{{ url('bm/provider/'.$document['provider_id'].'/document/'.$document['id'].'/update') }}"><span class="btn btn-success btn-large">Edit</span></a>
										<form style="display:inline-block; margin-left:20px;" action="{{ route('bm.provider.document.destroy', [$document['provider_id'], $document['id']]) }}" method="POST" id="form-delete">
