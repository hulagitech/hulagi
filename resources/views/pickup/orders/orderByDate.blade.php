        ...
        <div class="page-title-box">
            <div class="row align-items-center">
                ...
                <div class="col-md-8 d-flex justify-content-end">
                    @if(isset($dates))
                    <form class="form-inline pull-right" method="POST" action={{url('pickup/dateSearch')}}>
                        {{csrf_field()}}
                        ...
                        <script> req_id.push(<?php echo $request->id; ?>);</script>
                            <tr id="dataRow{{$index}}">
                                    ...
                                    @if($request->status)
                                        <div class='edit'>
                                            ...
                                            @elseif($request->status=="SORTCENTER")
                                                ...
                                                @endif
                                            @elseif($request->status=="NOTPICKEDUP")
                                                NOTPICKEDUP<BR>(CANCELLED)
                                            @else
                                                {{ @$request->status}} 
                                            @endif
                                        </div>
                                        @if($request->status=="PENDING"||$request->status=="NOTPICKEDUP"||$request->status=="ACCEPTED"||$request->status=="PICKEDUP")
                                        <select  class= 'txtedit' id='status-{{$index}}'>
                                            <option <?php if($request->status=="PENDING"){echo 'selected';} ?>>PENDING</option>
                                            <option <?php if($request->status=="NOTPICKEDUP"){echo 'selected';} ?>>NOTPICKEDUP</option>
                                ...
                                <td style="width:4%;">
                                    ...
                                    @else
                                        <label class="switch">
                                            <input type="checkbox" value="{{$request->cargo}}"  id='cargo-{{$request->id}}' disabled>
                                            <span class="slider round"></span>
                                        </label>
                                    @endif
                                </td>
