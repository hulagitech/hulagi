    ...
    <div class="col-sm-12">
        <div class="page-title-box">
            <div class="row align-items-center">
                <div class="col-md-8">
                    <h4 class="page-title m-0">Inbound Orders of Rider {{$RiderName[0]}}</h4>
                </div>
            ...
            <div class="card-body table-responsive">
                @if (count($inbound_orders) != 0)
                 <form class=" form-inline"  method="POST"
                    action="{{ route('pickup.bulkinvocie') }}" target="_blank">
                    {{csrf_field()}}
                    <table id="datatable" class="table table-bordered"
                            ...
                            <tr id="dataRow{{$index}}">
                                ...
                                <td style="width:4%;">
                                    @if($inbound_order->cargo)
                                        <label class="switch">
                                            <input type="checkbox" class="cargo" @if(isset($inbound_order->cargo)) @if(@$inbound_order->cargo=='1') checked @endif @endif id='cargo-{{$inbound_order->id}}'>
                                ...
                                <td>
                                    <div style="display:flex;">
                                        <a href="{{ route('pickup.order_details', $inbound_order->id) }}"
                        ...
                        <button type="submit" class="btn btn-lg btn-secondary pull-right fixed-btn" style=" font-size: 20px"><i class="fa fa-print text-danger"></i> Print the data </button> 
                </form>
                @else
     ...
     $( document ). ajaxComplete(function() {
            ...
            $(".allinbound").click(function(){
                if(confirm('Are you sure you want to Inbound ?')){ 
                    ...
                    $.ajax({
                        url: "{{url('pickup/inbound_order')}}/"+req_id,
                        type: 'post',
...
<script>
    $(document).ready(function(){
        ...
        $(".txtedit").focusout(function(){
            ...
            $.ajax({
                url: "{{url('pickup/requests')}}/"+edit_id,
                type: 'post',
...
<script>
    ...
    $(document).ready(function(){
        ...
        $(".cargo").click(function(){
            ...
            $.ajax({
                url: "{{url('pickup/changeCargo')}}/"+edit_id,
                type: 'post',
                data: field_name+"="+value,
                success:function(response){
                    console.log(response); 
