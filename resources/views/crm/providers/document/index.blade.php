            ...
            <div class="row">
                            ...
                            <tr>
                                ...
                                <td>
                                    <form action="{{ route('crm.provider.document.service', [$Provider->id, $service->id]) }}" method="POST">
                                        {{ csrf_field() }}
                                        {{ method_field('DELETE') }}
                                        <button class="btn btn-danger btn-large btn-block btn-secondary">Delete</a>
                                    </form>
                                </td>
                            </tr>
                ...
                <form action="{{ route('crm.provider.document.store', $Provider->id) }}" method="POST">
                    ...
                    <div class="col-xs-3" style="width: 12%;">
                        <button class="btn btn-success btn-block btn-secondary btn-rounded" type="submit">Update</button>
                    </div>
                </form>
            </div>
        </div>
					...
					<tbody>
						...
						<tr>
							...
							<td>
									...
									<a href="{{ url('crm/provider/'.$document['provider_id'].'/document/'.$document['id'].'/upload') }}" style="margin-left: 1em;" class="btn btn-success btn-secondary pull-right"><i class="fa fa-plus"></i> Add Doccument</a>
									
									@else
										<a href="{{ route('crm.provider.document.edit', [$document['provider_id'], $document['id']]) }}"><span class="btn btn-success btn-large btn-secondary">View</span></a>
										<a href="{{ url('crm/provider/'.$document['provider_id'].'/document/'.$document['id'].'/update') }}"><span class="btn btn-success btn-large btn-secondary">Edit</span></a>
										<form style="display:inline-block; margin-left:20px;" action="{{ route('crm.provider.document.destroy', [$document['provider_id'], $document['id']]) }}" method="POST" id="form-delete">
											<button class="btn btn-danger btn-large btn-secondary" form="form-delete">Decline</button>
											{{ csrf_field() }}
											{{ method_field('DELETE') }}
										</form>
