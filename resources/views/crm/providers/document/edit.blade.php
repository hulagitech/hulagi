            ...
            <div class="row">
                <div class="col-xs-6">
                    <form action="{{ route('crm.provider.document.update', [$Document['provider_id'], $Document['document_id'] ]) }}" method="POST">
                        {{ csrf_field() }}
                        {{ method_field('PATCH') }}
                        <button class="btn btn-block btn-success btn-secondary btn-rounded" type="submit">Approve</button>
                    </form>
                </div>
                <div class="col-xs-6">
                    <form action="{{ route('crm.provider.document.destroy', [$Document->provider_id, $Document['document_id']]) }}" method="POST">
                        {{ csrf_field() }}
                        {{ method_field('DELETE') }}
                        <button class="btn btn-block btn-danger btn-secondary btn-rounded" type="submit">Delete</button>
                    </form>
                </div>
            </div>
        </div>
