...
<div class="content-area py-1">
    <div class="container-fluid">
        <div class="box box-block bg-white">
            ...
            <a href="{{ route('fleet.provider.create') }}" style="margin-left: 1em;" class="btn btn-success btn-rounded btn-secondary pull-right"><i class="fa fa-plus"></i> Add New Driver</a>
                    ...
                    <tr>
                        ...
                        <td>
                            @if($provider->pending_documents() > 0 || $provider->service == null)
                                <a class="btn btn-danger btn-block label-right btn-secondary" href="{{route('fleet.provider.document.index', $provider->id )}}">Attention! <span class="btn-label">{{ $provider->pending_documents() }}</span></a>
                            @else
                                <a class="btn btn-success btn-block btn-secondary" href="{{route('fleet.provider.document.index', $provider->id )}}">All Set!</a>
                            @endif
                        </td>
                        <td>
                            @if($provider->service)
                                @if($provider->service->status == 'active')
                                    <label class="btn btn-block btn-primary btn-secondary">Yes</label>
                                @else
                        ...
                        <td>
                            <div class="input-group-btn">
                                ...
                                <a class="btn btn-danger btn-block btn-secondary" href="{{ route('fleet.provider.disapprove', $provider->id ) }}"><i class="fa fa-ban"></i></a>
                                @else
                                <a class="btn btn-success btn-block btn-secondary" href="{{ route('fleet.provider.approve', $provider->id ) }}"><i class="fa fa-check"></i></a>
                                @endif
                                <button type="button" 
                                    class="btn btn-black btn-block dropdown-toggle btn-secondary"
